...
from utils import get_all_examples, normalize_languages, random_id, bootstrap_confidence_interval, score_mgsm
...
def evaluate_forward_fn(args, forward_str, run_name="archive"):
    ...
    examples = get_all_examples(args.languages)
    ...
    if required_size > len(examples):
        selected_languages = ",".join(normalize_languages(args.languages))
        raise ValueError(
            f"Requested {required_size} MGSM examples but only {len(examples)} are available for languages: {selected_languages}."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--languages', type=str, default=','.join(normalize_languages()),
                        help='Comma-separated MGSM language codes to include, e.g. en or en,fr,zh')
    ...
    args.languages = [lang for lang in args.languages.split(',') if lang.strip()]
    # search
    SEARCHING_MODE = True
    search(args)
